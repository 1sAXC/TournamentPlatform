...
export function RegisterOrganizerPage() {
  const { register, handleSubmit, setError: setFieldError, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: { organizerName: '', email: '', contactHandle: '', password: '', confirm: '' },
  });
  const mutation = useRegisterOrganizerMutation();
  const navigate = useNavigate();
  const [error, setError] = useState<string | null>(null);

  const onSubmit = handleSubmit(({ organizerName, email, contactHandle, password }) => {
    setError(null);
    mutation.mutate({ organizerName, email, password, contactHandle: contactHandle.trim() }, {
  ...
  return (
    ...
    >
      ...
      <form className="col" style={{ gap: 14 }} onSubmit={onSubmit}>
        ...
        <Field label="Контакт для связи" hint="Telegram, Discord и т.п. — увидят капитаны команд для согласования матчей" error={errors.contactHandle?.message}>
