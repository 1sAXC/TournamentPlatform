...
public sealed class RatingOutboxMessageStore(RatingDbContext dbContext) : IOutboxMessageStore
{
    ...
    public async Task MarkProcessedAsync(Guid outboxMessageId, DateTime processedAtUtc, CancellationToken cancellationToken = default)
    {
        var message = await dbContext.OutboxMessages.FirstAsync(message => message.Id == outboxMessageId, cancellationToken);
